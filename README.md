...
3. **附件下载依赖临时链接。** 导出走的是 `table.getCellAttachmentUrls()`，链接 10 分钟有效，且跨域是否能 fetch 取决于飞书返回的 CORS 头。
   每个链接**失败会自动重试 1 次**，单张下载超时 **120 秒**，取地址超时 **60 秒**。
   **仍有失败时不会直接出文件** —— 会停在「有 N 张图片没下载成功」这一页，列出每张的文件名与失败原因，
   每条右侧有**刷新按钮**可单独重试，底部提供「全部重试」与「跳过，继续导出」；全部成功或选择跳过后才生成 Excel。
4. **单次 `addRecords` 上限 200 条**，插件已自动分批。`batchUploadFile` **禁止并发**，插件用全局串行链 + 每批 10 个（可在「导入选项」里调大）。
   **附件上传是整条链路里最慢的一步**：N 个附件要串行走 N/批 次网络往返，439 个附件通常要几分钟到十几分钟。这一步的瓶颈在**飞书接口与网络往返**，与本地 CPU / 内存无关 —— 换更快的电脑不会有改善。
   ⚠️ 上传接口在个别文件上会**挂起**（既不返回也不报错）。插件对每批设了 **90 秒超时**，降级为逐个上传时 **45 秒超时**；超时的文件记入「跳过的内容」后继续处理，不会让整个导入永久卡住。卡住时进度条会显示正在上传哪个文件。
